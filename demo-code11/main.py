...
class PoseEstimation:
    ...
    def update_data_panel(self, panel, keypoints, match_results):
        ...
        for category, templates in self.templates.items():
            ...
            frame.pack(fill="both", expand=True, pady=2)  # Adjusted pady to reduce row spacing
            ...
            for template in templates:
                ...
                match_info = tk.Label(match_info_frame,
                                      text=f"{template_name} Matches: {match_count} ({match_percentage:.2f}%)",
                                      font=("Arial", 12))
                match_info.pack(anchor="w", side="left")

                bar_length = int(match_percentage)  # Adjust the length of the bar based on match_percentage
                bar_frame = tk.Frame(match_info_frame, width=100, height=5, bg='white', relief="solid", bd=1)
                bar_frame.pack(anchor="w", side="left", padx=5, pady=0)  # Adjusted pady to reduce row spacing
                ...
                similarity_label = tk.Label(match_info_frame, text=f"{similarity:.2f}", fg='green', font=("Arial", 12))
                similarity_label.pack(anchor="w", side="left", padx=5)

                # Add a separator frame to ensure the next template starts on a new line
                separator = tk.Frame(frame, height=0)  # Adjusted height to reduce row spacing
