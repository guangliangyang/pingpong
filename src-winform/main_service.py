...
def main():
    ...
    video_file = os.path.join('..', 'mp4', '01.mov')
    pose_estimation = PoseEstimation()

    pose_estimation.analyze_video(video_file)
    ...
    print("speeds['overall']:",speeds['overall'])
    average_speed = speeds['overall']['avg']
    estimated_met = estimate_met(average_speed, step_count, swing_count)
    calories_burned = calculate_calories_burned(estimated_met, 70, total_time_minutes)
    calories_burned_per_hour, intensity = calculate_calories_burned_per_hour(calories_burned, total_time_minutes)

    print("Speed Statistics:")
    for direction, stats in speeds.items():
        print(f"{direction.capitalize()} Speed:")
        print(f"  Current: {stats['current']:.2f} m/s")
        print(f"  Max: {stats['max']:.2f} m/s")
        print(f"  Average: {stats['avg']:.2f} m/s")

    print("\nCalories Burned:")
    print(f"  Total Calories Burned: {calories_burned:.1f} kcal")
    print(f"  Average Calories Burned per Hour: {calories_burned_per_hour:.1f} kcal")
    print(f"  Intensity: {intensity}")

    highlight_ratios = {tuple(map(tuple, vertices)): 0 for vertices in pose_estimation.grid_rects}
    for cell_points_tuple in pose_estimation.highlight_counts:
        if cell_points_tuple in highlight_ratios:
            highlight_ratios[cell_points_tuple] = (pose_estimation.highlight_counts[cell_points_tuple] / sum(
                pose_estimation.highlight_counts.values())) * 100

    covered_area = pose_estimation.calculate_covered_area(highlight_ratios)

    print("\nGrid Count Bar Chart Data:")
    print(f"  Covered Area: {covered_area:.2f} m²")
    for cell_points, ratio in highlight_ratios.items():
        print(f"  Cell {cell_points}: {ratio:.1f}%")

    match_counts = pose_estimation.template_match_counts
    print("\nAction Statistics:")
    for category, templates in pose_estimation.templates.items():
        print(f"{category} Templates:")
        for template in templates:
            template_name = template['name']
            count = match_counts[category].get(template_name, 0)
