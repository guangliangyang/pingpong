...
class PoseEstimation:
    ...
    def calculate_covered_area(self):
        ...
        return covered_area
    def update_highlight_bar_chart(self, frame, highlight_ratios, chessboard_data,covered_area):
        # 创建一个图形对象，并设置尺寸
        ...
        # 添加图表标题
        ax.set_title(f'Covered Area: {covered_area}m²', fontsize=16, color='blue')
    ...
    def update_data_panel(self, panel, keypoints, match_results, speeds, swing_count, step_count, height_m):
        ...
        for category, templates in self.templates.items():
            ...
            title_text = ""
            if category == "Arm":
                title_text += f"Strokes:{swing_count}"
            elif category == "Footwork":
                title_text += f"Steps:{step_count}"
            ...
            bar_width = 0.5
            bars = ax1.barh(template_names, match_percentages, height=bar_width, color='gray', edgecolor='black')
            ax1.set_xlim(0, 100)
            ...
            # Set title with the same style as speed matrix
    ...
    def process_video(self, frame, pose):
        ...
        output_image_with_chart = self.update_highlight_bar_chart(output_image, highlight_ratios, chessboard_data, covered_area)
        return output_image_with_chart
            ...
            for foot_point in foot_points:
                ...
                for cell_points in chessboard_data['chessboard_vertices']:
                    ...
                    if self.is_point_in_quad((foot_x, foot_y), normalized_points):
                        ...
                        cv2.polylines(skeleton_canvas, [pts], isClosed=True, color=(0, 255, 255), thickness=1)
...
class PoseApp:
    ...
    def update_speed_stats(self, speeds):
        ...
        # 添加覆盖面积信息
        #coverage_label = tk.Label(self.speed_stats_panel, text=f"Coverage: {covered_area_size:.2f} m²",
        #                          font=("Arial", 16, "bold"), fg='blue')
        #coverage_label.grid(row=len(matrix_layout), column=0, columnspan=4, sticky="nsew")
