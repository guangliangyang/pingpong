...
class PoseApp:
    ...
    def update_data_plots(self):
        ...
        for category, data in self.data_memory.items():
            ...
            print(f"Updating data plots for category: {category}")  # 调试信息

            # Clear the existing figure and axes
            if self.data_panel_controls[category]['fig'] is not None:
                self.data_panel_controls[category]['fig'].clear()
                self.data_panel_controls[category]['canvas'].get_tk_widget().pack_forget()
