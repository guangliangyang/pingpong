...
class PoseEstimation:
    ...
    def update_data_panel(self, panel, keypoints, match_results, speeds, swing_count, step_count, height_m):
        ...
        norm, cmap = get_heatmap_settings()
        # 使用自定义的热力图设置
        norm, cmap = self.get_custom_heatmap_settings()
        ...
        for category, templates in self.templates.items():
            ...
            for i in range(len(template_names)):
                text_positions.append(i * 1.5)  # 缩小空隙
                extended_positions.append(template_names[i])
                extended_positions.append('')

            fig, ax1 = plt.subplots(figsize=(10, len(extended_positions) * 0.4))  # Increase the height of the figure

            colors = [cmap(norm(percentage)) for percentage in match_percentages]

            # Bar chart for match percentages with black border
            bar_width = 0.5
            ax1.barh([pos - bar_width * 0.9 for pos in text_positions], [100] * len(text_positions), height=0.5,
                     color='lightgray')
            bars = ax1.barh(text_positions, match_percentages, height=bar_width, color=colors, edgecolor='black')

            #bars = ax1.barh(text_positions, match_percentages, height=bar_width, color='gray', edgecolor='black')
            ax1.set_xlim(0, 100)

            # Add match percentage as text at the bar positions
            for i, (bar, percentage) in enumerate(zip(bars, match_percentages)):
                text_str = f'{template_names[i]}: {percentage:.2f}%'
                text_color = 'green' if template_names[i] in match_results[category] else 'black'
                y = bar.get_y() + bar_width  # 向下移动文字和条形图
                ax1.text(0, y- bar_width*0.3, text_str, va='center', ha='left', color=text_color, fontsize=12)
                bar.set_y(y - bar_width*1.9)  # 调整条形图的位置
...
class PoseApp:
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(self.left_frame, text="Footwork and Stroke Classification",
