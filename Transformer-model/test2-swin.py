...
# Swin Transformer 模型
class ActionSwinTransformer(nn.Module):
    def __init__(self, input_dim, num_classes, window_size=7):
        super().__init__()
        self.embedding = nn.Linear(input_dim, 49 * 96)  # 将输入映射到 49 * 96 维度
        self.swin_transformer = SwinTransformer(
            hidden_dim=96,
            layers=(2, 2, 6, 2),
            heads=(3, 6, 12, 24),
            channels=1,  # 输入通道为1，因为我们使用的是关键点数据
            num_classes=num_classes,
            head_dim=96,
            window_size=window_size,  # 自定义 window_size 以适应你的数据
        )

    def forward(self, src):
        print(f"Input shape before embedding: {src.shape}")
        src = self.embedding(src)
        print(f"Shape after embedding: {src.shape}")
        batch_size, seq_len, _ = src.size()
        src = src.view(batch_size * seq_len, 1, 7, 7 * 96)
        print(f"Shape after view: {src.shape}")
        output = self.swin_transformer(src)
        output = output.view(batch_size, seq_len, -1)
        return output


# 滑动窗口分析
def sliding_window_analysis(video_data, model, device, window_size=30, stride=30):
    ...
    for start in tqdm(range(0, num_frames - window_size + 1, stride), desc="滑动窗口分析"):
        ...
        frame_predictions.extend(preds[:stride])

    # 对最后一个窗口进行处理
    if start + stride < num_frames:
        remaining_frames = num_frames - (start + stride)
        ...
        with torch.no_grad():
            ...
            preds = output.argmax(dim=1).cpu().numpy()
        frame_predictions.extend(preds[-remaining_frames:])
...
# 数据准备函数
def prepare_data(video_data, frame_labels, window_size=30, stride=30):
    X, y = [], []
...
if __name__ == "__main__":
    ...
    window_size = 7  # 窗口大小设置为 7
    model = ActionSwinTransformer(input_dim=input_dim, num_classes=num_classes, window_size=window_size)
    model.to(device)
    ...
    for video_name, video_data, label_data in zip(processed_videos, all_video_data, all_frame_labels):
        ...
        if video_data.shape[0] < 30:  # 假设至少需要30帧
            ...
            continue
        # 进行滑动窗口分析
        frame_predictions = sliding_window_analysis(video_data, model, device, window_size=30, stride=30)
