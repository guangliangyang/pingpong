...
def draw_chessboard_on_frame(frame, chessboard_data, show_overlay=False, covered_area=None):
...
class IncrementalSpeedTracker:
    def __init__(self):
        self.total_speeds = {'forward': 0.0, 'sideways': 0.0, 'overall': 0.0}
        self.frame_counts = {'forward': 0, 'sideways': 0, 'overall': 0}
        self.recent_speeds = {'overall': []}  # 记录最近30次的速度

    def add_speed(self, speed_type,speed):
        if speed_type in self.total_speeds:
            self.total_speeds[speed_type] += speed
            self.frame_counts[speed_type] += 1

            if speed_type == 'overall':
                self.recent_speeds['overall'].append(speed)
                if len(self.recent_speeds['overall']) > 30:
                    self.recent_speeds['overall'].pop(0)

    def get_average_speed(self, speed_type):
        if speed_type in self.total_speeds:
            frame_count = self.frame_counts[speed_type]
            if frame_count == 0:
                return 0.0
            return self.total_speeds[speed_type] / frame_count
        return 0.0
...
class PoseEstimation:
    def __init__(self):
        ...
        self.speed_tracker = IncrementalSpeedTracker()
    ...
    def update_data_panel(self, panel, keypoints, match_results, speeds, swing_count, step_count, height_m):
        ...
        for category, templates in self.templates.items():
            ...
            # Create bar chart for template match counts and percentages
            template_names = [template['name'] for template in templates]
            match_counts = [self.template_match_counts[category].get(template['name'], 0) for template in templates]
            match_percentages = [(count / total_matches[category] * 100) if total_matches[category] > 0 else 0 for count
                                 in match_counts]

            fig, ax1 = plt.subplots(figsize=(10, len(templates) * 0.4))  # Increase the height of the figure

            # Bar chart for match percentages with reduced width
            bar_width = 0.5
            bars = ax1.barh(template_names, match_percentages, height=bar_width, color='skyblue', edgecolor='none')
            ax1.set_xlim(0, 100)

            # Add match percentage as text on y-axis side
            for i, (name, count, percentage) in enumerate(zip(template_names, match_counts, match_percentages)):
                text_str = f'{name} ({count}) {percentage:.2f}%'
                text_color = 'green' if name in match_results[category] else 'black'
                ax1.text(0, i, text_str, va='center', ha='left', color=text_color, fontsize=10,
                         weight='bold' if text_color == 'green' else 'normal')
        ...
        # 清除 frame01 中已有的内容
        for widget in self.app.frame01.winfo_children():
            widget.destroy()

        # Heatmap for highlight_ratios
        ...
        highlight_ratios_matrix = np.zeros((6, 3))  # Assuming a 6x3 grid for highlight_ratios

        for idx, vertices in enumerate(self.grid_rects):
            row = 5 - (idx // 3)  # 调整行顺序，使其从上到下递减
            col = idx % 3
            cell_tuple = tuple(map(tuple, vertices))
            highlight_ratios_matrix[row, col] = highlight_ratios[cell_tuple]

        fig, ax = plt.subplots(figsize=(6, 6))
        cax = ax.matshow(highlight_ratios_matrix, cmap='YlOrRd', vmin=0, vmax=100)
        ...
        for (i, j), val in np.ndenumerate(highlight_ratios_matrix):
            ax.text(j, i, f'{val:.2f}%', ha='center', va='center', color='black')

        ax.set_xticks([0, 1, 2])
        ax.set_xticklabels(['Far', 'Mid', 'Near'])
        ...
        # Adding color bar without ticks
        cbar = fig.colorbar(cax, ax=ax, aspect=10, shrink=0.75)
        cbar.ax.yaxis.set_ticks([])

        # Add title for heatmap
        covered_area_size = len(self.covered_area) * (self.large_square_width / 100) * (self.large_square_height / 100)
        ax.set_title(f'Covered Area Size: {covered_area_size:.2f} m²')

        # Place the heatmap_frame at the top of frame01
        heatmap_frame = tk.Frame(self.app.frame01)
    ...
    def update_video_panel(self, image, panel):
        window_width = int(screen_width * 0.5)  # 使用frame02的宽度
        window_height = int(screen_height * 0.48)  # 使用frame02中video_panel_frame的高度
    ...
    def process_video(self, frame, pose):
        ...
        if results.pose_landmarks:
            ...
            if self.previous_midpoint is not None:
                ...
                # 更新速度追踪器
                self.speed_tracker.add_speed('overall', current_speed['overall'])
                self.speed_tracker.add_speed('forward', current_speed['forward'])
                self.speed_tracker.add_speed('sideways', current_speed['sideways'])
...
class PoseApp:
    ...
    def setup_ui(self):
        ...
        left_frame_width = int(screen_width * 0.7)
        right_frame_width = int(screen_width * 0.3)
        ...
        # 计算新的 frame01 和 frame02 的宽度
        frame02_width = int(screen_width * 0.5)
        frame01_width = left_frame_width - frame02_width

        # 创建 frame01 和 frame02，并调整布局
        self.frame01 = ttk.Frame(self.left_frame, width=frame01_width, height=screen_height)
        self.frame01.pack(side="left", fill="y", expand=False)
        self.frame01.pack_propagate(False)  # 防止自动调整大小

        self.frame02 = ttk.Frame(self.left_frame, width=frame02_width, height=screen_height)
        self.frame02.pack(side="right", fill="y", expand=True)
        self.frame02.pack_propagate(False)  # 防止自动调整大小

        # 将 left_frame 原来的内容移到 frame02
        global mode_label
        mode_label = tk.Label(self.frame02, text="Mode: Real-time Analysis", bg="green", fg="white", font=("Arial", 16))
        mode_label.pack(side="top", fill="x")

        # 调整视频面板位置和大小
        video_panel_height = int(screen_height * 0.48)
        self.video_panel_frame = ttk.Frame(self.frame02, height=video_panel_height)
        self.video_panel_frame.pack(side="top", fill="both", expand=False)

        global video_panel
        video_panel = tk.Label(self.video_panel_frame)
        video_panel.pack(fill="both", expand=True)

        # 创建 frame0202 并调整位置和大小
        frame0202_height = int(screen_height * 0.48)
        self.frame0202 = ttk.Frame(self.frame02, height=frame0202_height)
        self.frame0202.pack(side="top", fill="both", expand=True)

        global skeleton_image_label
        skeleton_image_label = tk.Label(self.frame0202, bg="black")
        skeleton_image_label.pack(side="top", fill="both", expand=True)

        self.progress_frame = ttk.Frame(self.frame02, width=frame02_width, height=20)
        ...
        # 将speed_stats_panel移到data_panel下面
        self.speed_stats_panel = ttk.Frame(self.right_frame, width=right_frame_width, height=int(screen_height * 0.3))
        self.speed_stats_panel.pack(side="top", fill="both", expand=True)

        self.setup_speed_stats_panel()
    ...
    def setup_speed_stats_panel(self):
        # Adjust height for ECG-like graph
        self.ekg_fig, self.ekg_ax = plt.subplots(figsize=(5, 3))  # Reduced height
        self.ekg_fig.subplots_adjust(left=0.2, right=0.95)
        self.ekg_canvas = FigureCanvasTkAgg(self.ekg_fig, master=self.speed_stats_panel)
        self.ekg_canvas.get_tk_widget().pack(fill="x", expand=True)

        self.bar_fig, self.bar_ax = plt.subplots(figsize=(5, 3))
        self.bar_fig.subplots_adjust(left=0.2, right=0.95)
        self.bar_canvas = FigureCanvasTkAgg(self.bar_fig, master=self.speed_stats_panel)
        self.bar_canvas.get_tk_widget().pack(fill="x", expand=True)
    ...
    def update_speed_stats(self, speeds):
        ...
        # Clear the previous content of the axes
        self.ekg_ax.clear()
        self.bar_ax.clear()

        # Plot the ECG-like graph for the recent overall speeds
        recent_overall_speeds = [mps_to_kph(speed) for speed in
                                 self.pose_estimation.speed_tracker.recent_speeds['overall']]
        self.ekg_ax.plot(recent_overall_speeds, linestyle='-', color='r')
        self.ekg_ax.set_title('Recent Overall Speed')
        self.ekg_ax.set_xlabel('Frame')
        self.ekg_ax.set_ylabel('Speed (km/h)')
        self.ekg_ax.grid(True)
        self.ekg_ax.set_ylim(bottom=0)

        # Create the bar chart for the current, max, and average speeds
        categories = ['Forward', 'Sideways', 'Overall']
        current_speeds = [mps_to_kph(speeds['forward']['current']),
                          mps_to_kph(speeds['sideways']['current']),
                          mps_to_kph(speeds['overall']['current'])]
        max_speeds = [mps_to_kph(speeds['forward']['max']),
                      mps_to_kph(speeds['sideways']['max']),
                      mps_to_kph(speeds['overall']['max'])]
        avg_speeds = [mps_to_kph(speeds['forward']['avg']),
                      mps_to_kph(speeds['sideways']['avg']),
                      mps_to_kph(speeds['overall']['avg'])]

        bar_width = 0.2
        bar_positions = np.arange(len(categories))
        self.bar_ax.bar(bar_positions - bar_width, current_speeds, width=bar_width, label='Current')
        self.bar_ax.bar(bar_positions, max_speeds, width=bar_width, label='Max')
        self.bar_ax.bar(bar_positions + bar_width, avg_speeds, width=bar_width, label='Avg')

        self.bar_ax.set_title('Speed Statistics')
        self.bar_ax.set_xlabel('Speed Type')
        self.bar_ax.set_ylabel('Speed (km/h)')
        self.bar_ax.set_xticks(bar_positions)
        self.bar_ax.set_xticklabels(categories)
        self.bar_ax.legend()
        self.bar_ax.grid(True)
