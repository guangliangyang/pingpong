...
class PoseEstimation:
    ...
    def draw_skeleton(self, image, keypoints, connections, color, circle_radius=2):
        ...
        image_width = self.image_width  # 提前获取图像的宽和高
    ...
    def process_video(self, frame, pose):
        ...
        if results.pose_landmarks:
            ...
            if self.show_overlay:
                ...
                for cell_points, hit_count in cell_points_hits.items():
                    if hit_count > 0:
                        denormalized_points = [(int(pt[0] * self.image_width), int(pt[1] * self.image_height )) for pt in cell_points]
        ...
        if yolo_work:
            detected_objects, scaling_factor = detect_pingpong_table(frame, model)
        ...
        for step, duration in timers.items():
            ...
            # 以下是每一步的耗时统计
            # initial_setup: 0.0000 seconds
            # gpu_operations: 0.0005 seconds
            # pose_processing: 0.0215 seconds
            # update_dimensions: 0.0000 seconds
            # chessboard_processing: 0.0000 seconds
            # keypoints_processing: 0.0015 seconds
            # yolo_detection: 0.0000 seconds
            # final_processing: 0.2412 seconds
    ...
    def detect_pingpong_table(frame, model):
        ...
        return detected_objects, scaling_factor
    ...
    def analyze_video(self):
        ...
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while self.cap.isOpened() and self.video_playing:
                ...
                time_update_video_panel = time.time() - start_time


                start_time = time.time()
                root.update_idletasks()
                root.update()
                time_tk_update = time.time() - start_time
                ...
                # time.sleep(wait_time / 1000.0)  # 转换为秒并休眠

                # 输出每个部分的耗时
                print(f"Set Position Time: {time_set_pos:.4f}s, Read Frame Time: {time_read:.4f}s, "
                      f" Process Video Time: {time_process_video:.4f}s, "
                      f"Update Video Panel Time: {time_update_video_panel:.4f}s, "
                      f"Tkinter Update Time: {time_tk_update:.4f}s")
                #Set Position Time: 0.0311s, Read Frame Time: 0.0097s, Increment Frame Time: 0.0000s, Process Video Time: 0.2616s, Update Video Panel Time: 0.0123s
        self.video_playing = False
        self.cap.release()
    ...
    def update_skeleton_image(self, keypoints, match_results, foot_points, chessboard_data):
        screen_height = self.app.screen.get_height()
        screen_width = self.app.screen.get_width()
        top_height = int(screen_height * 2 / 3)  # 上方区域高度
        image_width = int(screen_width / GOLDEN_RATIO)  # 固定宽度为 pygame 窗口的一部分
        ...
        if arm_match:
            foot_coords = [(int(foot_point[0] * image_width), int(foot_point[1] * top_height)) for foot_point in
                           foot_points]
        ...
        if arm_match:
            # 创建一个字典来记录每个 cell_points 被命中的次数
            cell_points_hits = {
                tuple(map(tuple, [(int(pt[0] * image_width), int(pt[1] * top_height)) for pt in cell_points])): 0 for
                cell_points in chessboard_data['chessboard_vertices']}

            # 统计每个 chessboard_vertices 被 foot_coords 命中的次数
            for foot_x, foot_y in foot_coords:
                for cell_points in chessboard_data['chessboard_vertices']:
                    normalized_points = [(int(pt[0] * image_width), int(pt[1] * top_height)) for pt in cell_points]
                    if self.is_point_in_quad((foot_x, foot_y), normalized_points):
        ...
        # 颜色转换和图像旋转/翻转合并
        skeleton_image_np = skeleton_canvas
        #skeleton_image_np = cv2.cvtColor(skeleton_canvas, cv2.COLOR_BGR2RGB)
        skeleton_image_np = np.rot90(skeleton_image_np, -1)
        ...
        skeleton_surface = pygame.transform.scale(skeleton_surface, (image_width, top_height))  # 调整显示大小
        self.app.screen.blit(skeleton_surface, (int(screen_width / GOLDEN_RATIO), 0))
        pygame.display.update()
    ...
    def start_real_time_analysis(self):
        ...
        if self.cap is not None:
            self.cap.release()
        self.initialize_video_capture(0)  # 使用摄像头
