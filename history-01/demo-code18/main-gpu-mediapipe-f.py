...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # 指定使用第二个GPU（索引为1）
...
class PoseEstimation:
    ...
    def analyze_video(self, queue):
        ...
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5, use_gpu=True) as pose:
...
class PoseApp:
    ...
    def __init__(self, pose_estimation):
        ...
        # 获取布局
        self.layout = calculate_layout(self.window_width, self.window_height, title_label_height, video_height, left_ratio, mode_label_height)
    ...
    def update_mode_surface(self):
        ...
        self.mode_surface = self.create_label_surface(text, ("Arial", 22), "green", "white")
        ...
        self.screen.fill((0, 255, 0), rect=[region4_x, region4_y, region4_width, region4_height])
    ...
    def update_title_surface(self):
        ...
        self.title_surface = self.create_label_surface(title_text, ("Arial", 28), "green", "white")
        ...
        # 设置 region1 背景为绿色
        self.screen.fill((0, 255, 0), rect=[region1_x, region1_y, region1_width, region1_height])
    ...
    def update_skeleton_surface(self, skeleton_canvas):
        ...
        if self.current_layout == 1:
            ...
            skeleton_surface = pygame.transform.scale(skeleton_surface, (
                self.layout['region3']['width'], self.layout['region3']['height']))  # 调整显示大小
            self.screen.blit(skeleton_surface, (self.layout['region3']['x'], self.layout['region3']['y']))
    ...
    def update_data_panel(self, keypoints, match_results, speeds, swing_count, step_count, height_m):
        ...
        for category, templates in self.pose_estimation.templates.items():
            ...
            for template_name, percentage in zip(template_names, match_percentages):
                ...
                # 绘制进度条
                norm, cmap = get_heatmap_settings()
                color = cmap(norm(percentage))[:3]  # 使用百分比直接计算颜色
                color = tuple(int(c * 255) for c in color)
    ...
    def setup_ui(self):
        ...
        self.grid_count_bar_chart_ske = pygame.Surface((self.layout['region2']['width'], self.layout['region2']['height']))
    ...
    def on_key_press(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                ...
                self.layout['region3'], self.layout['region6'] = self.layout['region3'], self.layout['region6']
                #self.rebuild_ui()
