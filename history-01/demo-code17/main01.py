...
class PoseEstimation:
    ...
    def process_video(self, frame, pose):
        ...
        rect_points = [(500, 285), (850, 300), (1600, 700), (330, 720)]
        ...
        for col in range(1, cols):
            start_point = (int(rect_points[0][0][0] + col * (rect_points[1][0][0] - rect_points[0][0][0]) / cols),
                           int(rect_points[0][0][1] + col * (rect_points[1][0][1] - rect_points[0][0][1]) / cols))
            end_point = (int(rect_points[3][0][0] + col * (rect_points[2][0][0] - rect_points[3][0][0]) / cols),
                         int(rect_points[3][0][1] + col * (rect_points[2][0][1] - rect_points[3][0][1]) / cols))
            #cv2.line(image, start_point, end_point, (0, 0, 255), 2)
        for row in range(1, rows):
            start_point = (int(rect_points[0][0][0] + row * (rect_points[3][0][0] - rect_points[0][0][0]) / rows),
                           int(rect_points[0][0][1] + row * (rect_points[3][0][1] - rect_points[0][0][1]) / rows))
            end_point = (int(rect_points[1][0][0] + row * (rect_points[2][0][0] - rect_points[1][0][0]) / rows),
                         int(rect_points[1][0][1] + row * (rect_points[2][0][1] - rect_points[1][0][1]) / rows))
            #cv2.line(image, start_point, end_point, (0, 0, 255), 2)



        for (x, y) in rect_points.reshape(-1, 2):
            coord_text = f"({x}, {y})"
            #cv2.putText(image, coord_text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
        ...
        self.update_data_panel(data_panel, keypoints, match_results, covered_area, current_speed, max_speed, avg_speed,
    ...
    def update_skeleton_image(self, keypoints, match_results, foot_points, covered_area):
        ...
        rect_points = [(500, 285), (850, 300), (1600, 700), (330, 720)]
        #rect_points = [(500, 285), (850, 300), (1600, 700), (330, 720)]
        rect_points = [(int((x-200) * drawing_width / left_frame_width), int((y+200) * drawing_height / screen_height)) for x, y in
                       rect_points]
        rect_points = np.array(rect_points, np.int32).reshape((-1, 1, 2))
        cv2.polylines(skeleton_canvas, [rect_points], isClosed=True, color=(0, 0, 255), thickness=2)
        ...
        for col in range(1, cols):
            ...
            end_point = (int(rect_points[3][0][0] + col * (rect_points[2][0][0] - rect_points[3][0][0]) / cols),
                         int(rect_points[3][0][1] + col * (rect_points[2][0][1] - rect_points[3][0][1]) / cols))
            cv2.line(skeleton_canvas, start_point, end_point, (0, 0, 255), 2)
        for row in range(1, rows):
            start_point = (int(rect_points[0][0][0] + row * (rect_points[3][0][0] - rect_points[0][0][0]) / rows),
                           int(rect_points[0][0][1] + row * (rect_points[3][0][1] - rect_points[0][0][1]) / rows))
            end_point = (int(rect_points[1][0][0] + row * (rect_points[2][0][0] - rect_points[1][0][0]) / rows),
        ...
        #keypoints = [(x - 200, y+200, z) for (x, y, z) in keypoints]
