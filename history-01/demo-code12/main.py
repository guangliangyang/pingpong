...
class PoseEstimation:
    ...
    def update_data_panel(self, panel, keypoints, match_results, covered_area, current_speed, max_speed, avg_speed, swing_count, step_count):
        ...
        for category, templates in self.templates.items():
            ...
            title = tk.Label(frame, text=category.replace("_", " "), font=("Arial", 30, "bold"))
            title.pack(anchor="w")
        ...
        info_text = (f"Covered Area: {covered_area:.2f} m²\n"
                     ...
                     f"Swings Count: {swing_count} Steps Count: {step_count}")
        ...
        if results.pose_landmarks:
            ...
            if foot_points:
                ...
                if self.previous_foot_points is not None and self.previous_time is not None and time.time() - self.start_time > 3:
                    ...
                    current_speed = delta_distance / delta_time if delta_time > 0 else 0
    ...
    def update_skeleton_image(self, keypoints, match_results, foot_points, covered_area):
        ...
        margin_width = int(image_width * 0.01)
        margin_height = int(image_height * 0.01)
        ...
        if len(self.covered_area_points) == 4:
            ...
            cv2.polylines(skeleton_canvas, [points], isClosed=True, color=(255, 0, 0), thickness=2)
...
class PoseApp:
    ...
    def setup_ui(self):
        ...
        self.top_frame = ttk.Frame(self.right_frame, width=right_frame_width, height=int(screen_height * 0.6), relief="solid", borderwidth=1)
